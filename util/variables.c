...
void assign_variable_value(int var_index, const char *var_name, VarType expected_type, VarStorageType storage_type, void *value, size_t value_size) {
    if (var_index == -1) {  
        ...
        if (expected_type == STRING) memset(variables[var_index].value.string_val, 0, sizeof(variables[var_index].value.string_val));
        ...
        (expected_type == STRING) ? 
            (strncpy(variables[var_index].value.string_val, (char *)value, value_size), 
            variables[var_index].value.string_val[value_size] = '\0') :
        handle_error("Unsupported type");
    }
}